async (page) => {
  ...
  await button('Explorar categorías').click();
  if(await catalog().getByText('Lazo rosa',{exact:true}).count())throw new Error('Products displayed at root');
  await button(/^KIDS/).click();
  if(await catalog().getByText('Producto del padre',{exact:true}).count())throw new Error('Products displayed at intermediate level');
  ...
  await catalog().getByRole('combobox',{name:'Filtrar por categoría'}).selectOption('root');
  await catalog().getByText('Producto del padre',{exact:true}).waitFor();
  ...
  await catalog().getByText('Lazo rosa',{exact:true}).waitFor();
  await button('Explorar categorías').click();
  if(await catalog().getByPlaceholder('Buscar productos...').count())throw new Error('Explorer search visible before leaf');
  await button(/^Sin productos/).click();
  await catalog().getByText('No se encontraron productos',{exact:true}).waitFor();
  await button('Volver a la categoría anterior').click();
  ...
  await page.screenshot({path:'output/playwright/category-explorer-mobile.png'});
  await page.setViewportSize({width:1366,height:900});
  await page.screenshot({path:'output/playwright/category-explorer-desktop.png'});
  ...
  console.log('PASS: three views; explorer has no products until leaf; category dropdown, metaobject tree, 3 levels, parent products, no duplicates, uncategorized products, search/reset, remembered selection, empty categories, 5 widths, categories failure independent of all products.');
}
