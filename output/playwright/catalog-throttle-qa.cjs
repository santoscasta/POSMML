async page => {
 ...
 let pages=0; let throttled=false;
 ...
 await page.route('**/api/**',async route=>{
  ...
  if(path==='sessions/current') body={id:'qa',status:'OPEN',openingAmount:50};
   ...
   if(offset===30 && !throttled) {throttled=true;return route.fulfill({contentType:'application/json',body:JSON.stringify({errors:[{message:'Throttled'}]})});}
 ...
 if(pages!==4) throw new Error('Not all catalog pages loaded: '+pages);
