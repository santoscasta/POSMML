async(page)=>{
 ...
 await button('Imprimir informe').waitFor();
 if(await page.getByRole('dialog',{name:'Abrir sesión de caja'}).count())throw Error('Opening blocks report');
 await page.getByRole('dialog').getByRole('button',{name:'Cerrar',exact:true}).click();findings.push('QA-01 passed');
 session={id:'qa-session',status:'OPEN',openingAmount:50,cashierName:'QA',openedAt:'2026-09-14T08:00:00Z'};mode='orders-error';await page.reload();await login();await button('Pedidos').click();await page.getByRole('alert').filter({hasText:'No se pudieron cargar los pedidos'}).waitFor();
 if(await page.getByText('No hay pedidos',{exact:true}).count())throw Error('False empty orders');
 mode='normal';await button('Reintentar').click();await page.getByText('INITIAL',{exact:true}).waitFor();findings.push('QA-02 passed');
 mode='race';const search=page.getByPlaceholder('Buscar pedidos...');await search.fill('OLD');await page.waitForRequest(r=>r.url().endsWith('/graphql')&&r.postDataJSON()?.variables?.query==='OLD');await search.fill('NEW');await page.getByText('NEW',{exact:true}).waitFor();await page.waitForTimeout(2200);if(await page.getByText('OLD',{exact:true}).count())throw Error('Stale results');findings.push('QA-03 passed');
 mode='history-error';await button('Caja').click();await page.getByRole('alert').waitFor();if(await page.getByText('No hay sesiones anteriores',{exact:true}).count())throw Error('False empty history');mode='normal';await button('Reintentar historial').click();await page.getByText('No hay sesiones anteriores',{exact:true}).waitFor();findings.push('QA-04 passed');
