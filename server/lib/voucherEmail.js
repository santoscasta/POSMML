...
import { parseVoucherMetadata } from './voucherMetadata.js';
...
export async function sendVoucherEmail(gql, { id, email } = {}) {
  ...
  const data = await gql(`query PosEmailVoucher($id: ID!) { giftCard(id: $id) { id enabled note customer { id email } } }`, { id });
  ...
  if (!customer) {
    ...
    if (!customer) {
      const storedName = parseVoucherMetadata(card.note).customerName;
      const name = typeof storedName === 'string' ? storedName.trim() : '';
      const [firstName, ...lastNames] = name ? name.split(/\s+/) : [];
      customer = mutationResult(await gql(`mutation PosEmailCustomerCreate($input: CustomerInput!) {
      customerCreate(input: $input) { customer { id } userErrors { message } }
    }`, { input: { email, ...(firstName ? { firstName } : {}), ...(lastNames.length ? { lastName: lastNames.join(' ') } : {}) } }), 'customerCreate', 'customer');
    }
